%% RDKDC Final Project - ur5TransJacobianControl
%
% Written by Aabhas Jain
% Edits by Andrew Palacio
...
% Purpose:
    % implement a discrete-time Transpose Jacobian control system. It iterataively
    % implements the Transpose Jacobian control system discussed in class:
    % q_(k+1) = q_k - K*T_step*[transpose(Jb(q_k))]*xi_k
% Outputs: 
% finalerr: this should be -1 if there is a failure. if there is no
% failure then it should be the final positional error in cm.

function finalerr = ur5TransJacobianControl(gdesired, K, ur5)
T_step = 0.05;
v_abs_error = 1 / 100;
