...
classdef ur5_interface < handle
    ...
    properties (SetAccess = private, Hidden = true)
        ...
        ros_control_client
        switch_to_pos_ctrl_client
        switch_to_vel_ctrl_client
    ...
    methods
        ...
        function self = ur5_interface()
            ...
            self.switch_to_pos_ctrl_client = ros2svcclient(self.node_handle, "rdkdc/switch_to_pos_ctrl", 'std_srvs/Trigger');
            self.switch_to_vel_ctrl_client = ros2svcclient(self.node_handle, "rdkdc/switch_to_vel_ctrl", 'std_srvs/Trigger');
        ...
        % switch back to ROS control after students are done with
        ...
        function activate_pos_control(self)
            ...
            pos_control_req = ros2message(self.switch_to_pos_ctrl_client);
            try
                waitForServer(self.switch_to_pos_ctrl_client,"Timeout",1);
            catch
                error("Error: Velocity Functionality Not Enabled")
            end
            call(self.switch_to_pos_ctrl_client,pos_control_req,"Timeout",1).success;
        end
        ...
        function activate_vel_control(self)
            ...
            % send a request to switch the controllers
            vel_control_req = ros2message(self.switch_to_vel_ctrl_client);
            try
                waitForServer(self.switch_to_vel_ctrl_client,"Timeout",1);
            catch
                error("Error: Velocity Functionality Not Enabled")
            end
            call(self.switch_to_vel_ctrl_client,vel_control_req,"Timeout",1).success;
        end
